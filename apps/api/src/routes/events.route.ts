...
import { createEventBodySchema, eventIdParamsSchema, getEventsCollectionQuerySchema } from "../schemas/events.schemas";
...
router.post(
    ...
    async (req, res) => {
        const event = await eventsService.createEvent(req.body, req.sessionData!.user.id);
